...
<body>
  <div class="topbar">
    <div class="topbar-inner">
      ...
      <div class="controls">
        <span class="pill" id="pillStatus">Statut: <strong id="statusText">…</strong></span>
  ...
  <div class="wrap">
    <div class="grid">
      ...
      <div class="card">
        ...
        <div class="search">
          <input id="searchInput" placeholder="Rechercher (id, user_id, preview)…" />
      ...
      <div class="card pane">
        <div class="card-header">
          ...
          <div style="display:flex; gap:8px; align-items:center; flex-wrap:wrap;">
            <span class="chip" id="brandChip">brand: —</span>
            <span class="chip" id="userChip">user_id: —</span>
          </div>
        </div>
        ...
        <div class="pane-body">
          <div class="empty" id="emptyState">
            ...
            <div style="margin-top:10px" class="small">
              Astuce: si le token admin n’est pas en URL, vous pouvez le mettre via le champ en bas, puis <span class="kbd">Rafraîchir</span>.
            </div>
          </div>
        ...
        <div class="toolbar">
          <div class="field" title="Token admin (X-Admin-Token / ?token=...)">
            ...
            <input id="tokenInput" placeholder="ADMIN_DASHBOARD_TOKEN (optionnel si déjà en URL)" />
          </div>
  ...
  <script>
    ...
    async function loadDiag(){
      ...
      $("statusText").textContent = ok ? "OK" : "Dégradé";
    ...
    function applyFilter(){
      ...
      state.filtered = state.conversations.filter(c => {
        ...
        return id.includes(q) || userId.includes(q) || prev.includes(q) || brand.includes(q);
      });
    }

    function brandChipClass(brand){
      if(brand === "runningman") return "chip runningman";
      return "chip retroworld";
    }
    ...
    function renderList(){
      ...
      for(const c of state.filtered){
        ...
        div.innerHTML = `
          ...
          <div class="meta">
            <span>🕒 <code>${escapeHtml(ts || "—")}</code></span>
            <span>👤 <code>${escapeHtml(c.user_id || "—")}</code></span>
          </div>
        `;
        ...
        div.addEventListener("click", async () => {
          ...
          $("thread").innerHTML = `<div class="empty">Chargement…</div>`;
          try{
    ...
    function renderThread(){
      ...
      const brand = (state.activeObj && state.activeObj.brand_last) ? String(state.activeObj.brand_last) : "—";
      const userId = (state.activeObj && state.activeObj.user_id) ? String(state.activeObj.user_id) : "—";
    ...
    $("btnReload").addEventListener("click", async () => {
      if(!state.activeId){ toast("Aucune conversation sélectionnée"); return; }
      try{
        await loadConversationDetail(state.activeId);
    ...
    $("btnCopyJson").addEventListener("click", async () => {
      if(!state.activeId || !state.activeObj){ toast("Aucune conversation sélectionnée"); return; }
      try{
        const payload = JSON.stringify(state.activeObj, null, 2);
        await navigator.clipboard.writeText(payload);
        toast("JSON conversation copié");
      }catch(e){
        toast("Impossible de copier");
      }
    });
